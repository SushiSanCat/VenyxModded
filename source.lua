...
do
	function utility:Create(instance, properties, children)
		...
		if instance == "ImageLabel" and properties.ImageColor3 == themes.Background then
			local gradient = Instance.new("UIGradient")
			gradient.Color = ColorSequence.new({
				ColorSequenceKeypoint.new(0, themes.GradientTop),
				ColorSequenceKeypoint.new(1, themes.GradientBottom)
			})
	...
	function utility:Tween(instance, properties, duration, ...)
		-- Add scale and fade animation for smoother transitions
		if properties.Size then
			local scaleTween = tween:Create(instance, tweeninfo(duration, ...), {Size = properties.Size})
			scaleTween:Play()
		end
		if properties.ImageTransparency ~= nil then
			local fadeTween = tween:Create(instance, tweeninfo(duration, ...), {ImageTransparency = properties.ImageTransparency})
			fadeTween:Play()
		end
		if properties.Position then
			local posTween = tween:Create(instance, tweeninfo(duration, ...), {Position = properties.Position})
			posTween:Play()
		end
		for k, v in pairs(properties) do
			if k ~= "Size" and k ~= "ImageTransparency" and k ~= "Position" then
				tween:Create(instance, tweeninfo(duration, ...), {[k] = v}):Play()
			end
		end
	end
...
do
	...
	function library:Notify(title, text, callback)
		...
		utility:Tween(notification, {ImageTransparency = 0}, 0.3)
		utility:Tween(notification, {Size = UDim2.new(0, textSize.X + 70, 0, 60)}, 0.3)
		wait(0.3)
